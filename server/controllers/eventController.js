...
const eventController = {
  ...
  updateStatusEvent: async (req, res) => {
      const updateStatus = await Event.findByIdAndUpdate(req.params.id, {status: req.body.status},
        (err, data)=>{
          err?res.status(500).send(err.message):res.status(200).send("update status success");
        });
  },
  ...
  getAllEvents: async (req, res) => {
    ...
    .then(async events =>{
      if(events){
        let allEvent =await Promise.all(events.map( async data =>{
          let details = await EventDetail.find({eventId: data._id})
          .then(eventD=>{
            return eventD;
          });
